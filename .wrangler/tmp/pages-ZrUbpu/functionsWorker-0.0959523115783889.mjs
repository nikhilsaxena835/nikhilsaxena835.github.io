...
async function onRequestGet(context) {
  try {
    ...
    const dailyAlbum = await db.prepare(`${ALBUM_QUERY} WHERE al.id = (SELECT id FROM albums LIMIT 1 OFFSET ?) GROUP BY al.id`).bind(offset).first();
    if (!dailyAlbum) {
      return Response.json({ error: "No albums available" }, { status: 404 });
    }
    const interaction = await db.prepare("SELECT status FROM UserAlbum WHERE username = ? AND album_id = ?").bind(username, dailyAlbum.id).first();
    if (interaction) {
...
async function onRequestGet3(context) {
  try {
    ...
    const countries = await db.prepare("SELECT DISTINCT country FROM artists WHERE country IS NOT NULL ORDER BY country").all();
    const genres = await db.prepare("SELECT DISTINCT name FROM genres ORDER BY name").all();
    const decades = await db.prepare(
...
async function onRequestPost(context) {
  try {
    ...
    ).bind(username, album_id, status).run();
...
async function onRequestGet6(context) {
  try {
    ...
    const totalCountries = await db.prepare("SELECT COUNT(DISTINCT country) as count FROM artists WHERE country IS NOT NULL").first();
    const counts = await db.prepare(
      "SELECT status, COUNT(*) as count FROM UserAlbum WHERE username = ? GROUP BY status"
    ).bind(username).all();
    const genresExplored = await db.prepare(
...
async function onRequestPost2(context) {
  try {
    ...
    const existing = await db.prepare("SELECT username, created_at FROM User WHERE username = ?").bind(lower).first();
    if (existing) {
      return Response.json({ user: existing, isNew: false });
    }
    await db.prepare("INSERT INTO User (username) VALUES (?)").bind(lower).run();
    const user = await db.prepare("SELECT username, created_at FROM User WHERE username = ?").bind(lower).first();
...
async function onRequestGet7(context) {
  try {
    ...
    const db = context.env.DB;
    const allCountries = await db.prepare("SELECT DISTINCT country FROM artists WHERE country IS NOT NULL ORDER BY country").all();
    ...
    let album = await db.prepare(
      `SELECT al.id, al.title, al.release_year, al.cover_art_url,
         ...
         GROUP BY al.id
         ORDER BY RANDOM() LIMIT 1`
    ).bind(nextCountry, username).first();
    if (!album) {
      for (const fallbackCountry of unvisited) {
        album = await db.prepare(
          `SELECT al.id, al.title, al.release_year, al.cover_art_url,
             ...
             GROUP BY al.id
             ORDER BY RANDOM() LIMIT 1`
        ).bind(fallbackCountry, username).first();
        if (album) break;
      }
    }
    if (!album) {
...
// ../.wrangler/tmp/pages-ZrUbpu/functionsRoutes-0.46231519409900756.mjs
var routes = [
  {
    routePath: "/api/daily",
    mountPath: "/api",
...
// ../.wrangler/tmp/bundle-5R0puI/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-5R0puI/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.0959523115783889.mjs.map
